...
fn countInterp(roots: []const Tree.Node) usize {
    var ix = 0;
    for (roots) |node| {
        switch (node.kind) {
            .interp => ix += 1,
...
fn captureInterps(roots: []const Tree.Node, out: [][:0]const u8, ix: *usize) void {
    for (roots) |node| {
        switch (node.kind) {
            .interp => {
                out[ix.*] = node.raw ++ "";
...
pub fn Templ(comptime id: Atom, comptime template: []const u8) !type {
    ...
    return struct {
        ...
        fn renderNodes(comptime nodes: []const Tree.Node, writer: anytype, args: anytype) !void {
...
const Engine = struct {
    templates: []const Document,

    pub fn init(comptime templates: anytype) Engine {
        const ti = @typeInfo(templates).@"struct";
        comptime std.debug.assert(!ti.is_tuple);
    ...
    pub fn render(
        comptime self: Engine,
        w: *std.Io.Writer,
        comptime template: @EnumLiteral(),
        link_args: anytype,
        args: anytype,
    ) !void {
        _ = self;
...
test {
    const tmpl: Engine = .init(.{
        .header =
        \\<header | <div|@.content@>>
        ,
        .example =
        \\<a @.splat_attr@ .@.class_value@ #@.id_value@ data-value=@.data-value@|
        \\  @header@
        \\  @.interpolated_value@
        \\  @if .boolean_value {
        \\    <div| render me conditionally>
        \\  }@
        \\  <div|
        \\    @if .another_boolean_value {
        \\        One
        \\    } else {
        \\        Or the Other!
        \\    }@
    ...
    var buf =  std.testing.allocator.alloc(u8, 4096);
    ...
    try tmpl.render(
        ...
        .{
            ...
            .class_value = "bg-red h-full",
            .id_value = "my-value",
            .@"data-value" = "345",
            .interpolated_value = "Hello, World!",
            .boolean_value = true,
            .another_boolean_value = true,
            .iterable_value = &.{
                "a",
                "b",
                "c",
