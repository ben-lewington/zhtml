...
pub fn Tokeniser(comptime config: struct {
    ...
    const Keywords = b: {
        _ = @typeInfo(config.Keywords).@"enum";
        break :b config.Keywords;
    };
    const Symbols = b: {
        const enum_ti = @typeInfo(config.Symbols).@"enum";
        if (enum_ti.tag_type != u8) @compileError("symbol enum must have a u8 representation");
        break :b config.Symbols;
    };
    const syms: []const u8 = b: {
        const fs = @typeInfo(Symbols).@"enum".fields;
        var s: [fs.len]u8 = undefined;
        for (fs, 0..) |f, i| s[i] = f;
        break :b s[0..fs.len];
    };
    ...
    return struct {
        ...
        pub fn peekNextTok(self: *const Toks) Token.Info {
            ...
            } else if (std.enums.fromInt(Symbol, self.input[trim])) |sym| {
                ix += 1;
            ...
            chop = while (ix < eof) : (ix += 1) {
                ...
                const raw = self.input[trim..];
                if (std.meta.stringToEnum(Keyword, raw)) |kw| {
                    return .{ .tok = .{
                        .raw = raw,
                        .kind = .keyword,
                        .keyword = kw,
            ...
            const raw = self.input[trim..chop];
            var kind = .token;
            if (std.meta.stringToEnum(Keyword, raw)) |kw| {
                self.keyword = kw;
                kind = .keyword;
